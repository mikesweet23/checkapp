...
export default function ResultsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const reportMessage = delivery?.emailStatus === "sent" && delivery.pdfStatus === "generated"
    ? `Your full PDF report has been sent to ${participant?.email}.`
    : delivery?.emailStatus === "sent"
      ? `Your report has been sent to ${participant?.email}.`
      : delivery?.emailStatus === "failed"
        ? "We could not send the report automatically. Please contact Absolute Mind for help."
        : `Your result is ready. Report delivery will be connected for ${participant?.email}.`;
