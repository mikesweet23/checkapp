...
export async function sendResultEmail({ participant, assessment, result }: {
  ...
  const categoryRows = assessment.categories.map((category) => `<tr><td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;color:#555">${escapeHtml(category.name)}</td><td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;text-align:right;font-weight:700;color:#f16334">${result.categoryScores[category.id] ?? 0}%</td></tr>`).join("");
  const html = `<div style="background:#f7f7f7;padding:32px 16px;font-family:Montserrat,Arial,sans-serif;color:#444"><div style="max-width:620px;margin:0 auto;background:white;border:1px solid #e5e5e5;border-radius:18px;padding:32px"><p style="color:#f16334;font-weight:700;letter-spacing:.08em;text-transform:uppercase;font-size:12px">Absolute Mind · Personalised report</p><h1 style="font-size:30px;line-height:1.1;margin:16px 0 10px">${firstName}, here is your ${escapeHtml(assessment.shortName)} result</h1><p style="font-size:18px;margin:0 0 24px"><strong>${result.overall}/100</strong> · ${bandTitle}</p><p style="font-size:16px;line-height:1.6">${bandSummary}</p><p style="font-size:16px;line-height:1.6">${bandBody}</p><h2 style="font-size:20px;margin-top:28px">Your areas at a glance</h2><table style="width:100%;border-collapse:collapse;font-size:15px">${categoryRows}</table><p style="margin-top:28px"><a href="${escapeHtml(result.band.ctaHref)}" style="display:inline-block;background:#f16334;color:white;text-decoration:none;padding:13px 18px;border-radius:999px;font-weight:700">${escapeHtml(result.band.ctaLabel)}</a></p><p style="font-size:12px;line-height:1.5;color:#666;margin-top:28px">This check-in is for reflection and is not a diagnosis. If you feel at risk or need urgent support, contact an appropriate healthcare or emergency service.</p><p style="font-size:13px;line-height:1.5;color:#666;margin-top:22px">Your full branded report is attached as a PDF.</p></div></div>`;
